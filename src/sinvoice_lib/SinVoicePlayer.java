	...
	private boolean convertTextToCodes(String text) {
		...
		if (!TextUtils.isEmpty(text)) {
			...
			for (int i = 0; i < len; ++i) {
				...
				int index = -1;
				try {
					index = mCodeBook.indexOf(ch);
				} catch (NullPointerException e) {
				}
				if (index > -1) {
					mCodes.add(index);//传递的是字符在表中的位置
				}else if(index==-1&&ch==Common.FENGE)
				{
					mCodes.add(Common.FENGE_TOKEN);//若遇到分隔符，直接传-4
				}
				else {
					System.out.println("faile  "+ch);
					ret = false;
					LogHelper.d(TAG, "invalidate char:" + ch+" "+index+"   codebook="+mCodeBook);
					break;
				}
			}
			if (ret) {
...
//			System.out.println("convertTextToCodes(text) failed!");
//		}
			...
			mEncodeThread = new Thread() {
				...
				public void run() {
					do {
						...
						mEncoder.encode(mCodes, Common.DEFAULT_GEN_DURATION,
								muteInterval);
