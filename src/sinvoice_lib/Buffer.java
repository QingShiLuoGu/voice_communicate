...
//        printAllQueue();
    ...
    public void printAllQueue() {
        ...
        for (int i = 0; i < size; ++i) {
            BufferData data = mProducerQueue.peek();
//            System.out.println("mProducerQueue "+i+" "+ mProducerQueue.poll().mData[1]);
            if (null == data || null == data.mData) {
//                mProducerQueue.poll();
            }
        }
        ...
        size = mConsumeQueue.size();
        for (int i = 0; i < size; ++i) {
            BufferData data = mConsumeQueue.peek();
//            System.out.println("mConsumeQueue "+i+" "+data.mData);
        }

        LogHelper.d(TAG, "printAllQueue ProducerQueue Size:" + mProducerQueue.size() + "    ConsumeQueue Size:" + mConsumeQueue.size());
    }
